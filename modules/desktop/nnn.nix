...

{
  ...
  services.greetd = {
    ...
    settings = {
      default_session = {
        ...
        command = "${pkgs.tuigreet}/bin/tuigreet --time --greeting \"\" --remember --remember-session --asterisks --sessions ${config.services.displayManager.sessionData.desktops}/share/wayland-sessions";
        user = "greeter";
      };
    };
  };
  ...
  environment.sessionVariables = {
    ...
    # Route Firefox hardware video decode through NVIDIA's direct NVDEC path.
    # Without this, Firefox ignores LIBVA_DRIVER_NAME on Wayland + NVIDIA.
    NVD_BACKEND = "direct";
